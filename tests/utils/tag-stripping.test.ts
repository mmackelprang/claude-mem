...
import { stripMemoryTags, isInternalProtocolPayload, stripTags } from '../../src/utils/tag-stripping.js';
...
after`;
  ...
  describe('Phase 2 fail-safe: any WRAPPING private* tag redacts, self-closing switch survives', () => {
    ...
    it('plain <private> still redacts exactly as before (non-regression)', () => {
      // Upstream collapsed stripMemoryTagsFromPrompt/FromJson (both were literally
      // `stripTags(content).stripped`) into the single stripMemoryTags. Pure rename.
      expect(stripMemoryTags('public <private>secret</private> more')).toBe('public  more');
      expect(stripMemoryTags('<private>entire prompt</private>')).toBe('');
    });
