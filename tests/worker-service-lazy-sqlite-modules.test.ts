...
const SESSION_STORE_PATH = join(PLUGIN_DIR, 'sqlite', 'SessionStore.cjs');
...
// createRequire('../sqlite/SessionStore.cjs') that the build must emit as a loose
// sibling (Approach A — SessionStore pulls bun:sqlite, so it cannot be inlined
// into the SDK bundle). The module ships with a .cjs extension (not .js) so Node
// consumers can require() it despite plugin/package.json's "type":"module". parseFileList,
// by contrast, is now a static import inlined into the worker bundle (Approach B),
// so it must resolve WITHOUT any plugin/sqlite/observations/files.js file existing.
describe('worker-service.cjs lazy/inlined SQLite modules (#3091/#3092/#3107)', () => {
  it('emits sqlite/SessionStore.cjs next to the worker bundle', () => {
    expect(existsSync(SESSION_STORE_PATH)).toBe(true);
  });

  it('resolves sqlite/SessionStore.cjs the same way ChromaSync.ts requires it at runtime', () => {
    const { SessionStore } = require(join(WORKER_SCRIPTS_DIR, '../sqlite/SessionStore.cjs'));
