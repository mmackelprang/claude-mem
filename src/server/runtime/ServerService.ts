...
interface CliFlagValues {
  ...
  status?: string;
  active?: boolean;
  // WS2 author seam — `--actor <id>` provisions teammate keys with distinct
  // author identities (see resolveServerApiKeyCliActorId). MUST also be declared
  // in parseFlagArgs' options below: under node's parseArgs an UNDECLARED
  // `--actor mark` parses as `{ actor: true }` with 'mark' falling through as a
  // positional, even with strict:false.
  actor?: string;
}
