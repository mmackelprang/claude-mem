  ...
  return withPostgresTransaction(input.pool, async (client) => {
    ...
    for (let index = 0; index < rendered.length; index++) {
      ...
      const observation = await obsRepo.create({
        ...
        // Phase 1 (WS2 author seam) — denormalize author identity onto the
        // observation row itself so team memory is attributable/filterable by
        // teammate without a five-hop audit join.
        actorId: input.actorId ?? null,
        apiKeyId: input.apiKeyId ?? null,
        // Phase 2 (WS2 visibility seam) — fail CLOSED. If a caller ever omits
        // visibility entirely, persist 'private' rather than inheriting the
        // column's 'team' default: private-by-mistake beats leaked.
