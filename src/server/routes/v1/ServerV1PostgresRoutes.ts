  ...
  setupRoutes(app: Application): void {
    ...
    app.post('/v1/memories', writeAuth, this.handleCreate(
      ...
      async (req, res, body) => {
        ...
        const createInput = {
          ...
          // Phase 1 (WS2 author seam) — attribute the manual write.
          actorId: await this.resolveActorId(req),
          apiKeyId: req.authContext?.apiKeyId ?? null,
          // Phase 2 (WS2 visibility seam) — an explicit body.visibility wins
          // (the Phase-3 SQLite→Postgres importer stamps 'private' per row);
          // otherwise the config-driven default applies.
    ...
    app.post('/v1/search', readAuth, this.handleCreate(
      ...
      async (req, res, body) => {
        ...
        const platformSource = normalizePlatformSourceOrNull(body.platformSource);
        try {
          // Phase 4 — Chroma-first recall with FTS fallback. filter.viewerActorId
    ...
    const mcpHandler = this.asyncHandler(async (req, res) => {
      ...
      const backend: RecallBackend = {
        ...
        context: async ({ projectId, query, limit }) => {
          assertProjectAllowed(projectId);
          // Phase 4 — Chroma-first recall with FTS fallback, same visibility
          // predicate as POST /v1/context via filter.viewerActorId.
          const result = await this.recall.search({ projectId, teamId, query, limit, filter: { viewerActorId } });
          await this.auditWrite(req, 'observation.read', null, projectId, {
        ...
        recent: async ({ projectId, limit }) => {
          assertProjectAllowed(projectId);
          // Phase 2 — viewerActorId is the visibility predicate; without it this
          // MCP 'recent' path would list other actors' private observations.
